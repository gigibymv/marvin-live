...
export function mapGateReviewPayloadToModal(
  ...
  return {
    ...
    merlinVerdict: Object.keys(merlinVerdict).length ? {
      ...
      recommendation_enum: asString(merlinVerdict.recommendation_enum) ?? asString(merlinVerdict.verdict) ?? null,
      conditions: asStringArray(merlinVerdict.conditions),
      deal_breakers: asStringArray(merlinVerdict.deal_breakers),
      final_thesis: asString(merlinVerdict.final_thesis) ?? null,
